...
class Problem(models.Model):
    ...
    def update_stats(self):
        self.user_count = self.submission_set.filter(points__gte=F('problem__points')).values('user').distinct().count()
        submissions = self.submission_set.count()
        self.ac_rate = 100.0 * self.submission_set.filter(points__get=F('problem__points')).count() / submissions if submissions else 0
        self.save()
