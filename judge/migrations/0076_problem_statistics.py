...
def compute_user_count(apps, schema_editor):
    ...
    for problem in Problem.objects.using(schema_editor.connection.alias):
        problem.user_count = problem.submission_set.filter(points__gte=F('problem__points')).values('user').distinct().count()
        problem.ac_rate = 100.0 * self.submission_set.filter(points__gte=F('problem__points')).count() / submissions if submissions else 0
        problem.save()
...
def compute_old_user_count(apps, schema_editor):
    ...
    for problem in Problem.objects.using(schema_editor.connection.alias):
        ...
        problem.ac_rate = 100.0 * self.submission_set.filter(result='AC').count() / submissions if submissions else 0
        problem.save()
